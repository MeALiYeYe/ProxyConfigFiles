...
uninstall_substore() {
    ...
    if command -v lsof >/dev/null; then
        local pid_3000=$(lsof -ti:3000 2>/dev/null || true)
        if [ -n "$pid_3000" ]; then
            log_info "清理端口 3000（PID: $pid_3000）..."
            kill -9 $pid_3000 2>/dev/null || true
        fi
        
        # 获取占用3001端口的进程并杀死
        local pid_3001=$(lsof -ti:3001 2>/dev/null || true)
        if [ -n "$pid_3001" ]; then
            log_info "清理端口 3001（PID: $pid_3001）..."
            kill -9 $pid_3001 2>/dev/null || true
        fi
    else
        # 如果没有 lsof，使用 fuser 作为备选方案
        log_warn "lsof 不可用，尝试使用 fuser 清理端口..."
    ...
    if [ -d "$SUBSTORE_DIR" ]; then
        rm -rf "$SUBSTORE_DIR"
        log_info "Sub-Store 目录已删除"
    else
        log_warn "Sub-Store 目录不存在"
    fi
    
    log_info "✅ Sub-Store 已完全卸载（端口已清理）"
}
...
uninstall_script() {
    log_info "卸载 SubMi.sh..."
    
    if [ -f "$HOME/bin/SubMi.sh" ]; then
        rm -f "$HOME/bin/SubMi.sh"
    fi
    
    if [ -f "$HOME/bin/SubMi.sh.bak" ]; then
        rm -f "$HOME/bin/SubMi.sh.bak"
    fi
    
    if [ -L "$HOME/.termux/boot/SubMi.sh" ]; then
        rm -f "$HOME/.termux/boot/SubMi.sh"
    fi
    
    log_info "SubMi.sh 已卸载"
}
