 ...
 * * 🔗 原作者 https://raw.githubusercontent.com/jnlaoshu/MySelf/master/Egern/Widget/GasPrice.js
 * ==========================================
 */
...
async function loadPrediction(ctx, provinceCode) {
  ...
  try {
    ...
    // ===== 使用第一个脚本的解析逻辑（增强版）=====
    const reg_adjust = /<div class="tishi"> <span>(.*)<\/span><br\/>([\s\S]+?)<br\/>/;
    const m = html.match(reg_adjust);

    if (!m) return null;

    let raw = m[2];

    const up = raw.includes('上调') || raw.includes('上涨');
    const down = raw.includes('下调') || raw.includes('下跌');

    // ===== 优先解析：元/升区间 =====
    const range = raw.match(/([\d\.]+)元\/升-([\d\.]+)元\/升/);
    if (range) {
      return {
        up,
        down,
